...
exports.getAllUserBoardGames = async (req, res) => {
    ...
    try {
        const result = await db.query('SELECT * FROM Board_Games WHERE user_id = $1', [userId]);
        res.status(200).json(result.rows);
    } catch (error) {
        console.error(error);
...
exports.getBoardGameById = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query('SELECT * FROM Board_Games WHERE product_id = $1', [id]);
...
exports.updateBoardGame = async (req, res) => {
    ...
    try {
        ...
        if (productResult.rows.length === 0) {
            return res.status(404).json({ error: 'Board game not found in Products' });
        }
        ...
        res.status(200).json({
            ...
            boardGame: boardGameResult.rows[0]
        });
    } catch (error) {
        console.error(error);
...
exports.deleteBoardGame = async (req, res) => {
    ...
    try {
        ...
        const boardGameResult = await db.query('DELETE FROM Board_Games WHERE product_id = $1 RETURNING *', [id]);

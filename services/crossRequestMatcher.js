...
//         SELECT user_id,'match_found', $2, 'Unread'
...
// 📁 services/crossRequestMatcher.js
const db = require("../config/db");
const { emitCrossRequestMatch } = require("./socketEmitter"); // ✅ ייבוא חדש
...
async function findAndInsertCrossMatchesForRequest(requestId) {
  ...
  try {
    ...
    for (const match of matches) {
      ...
      await client.query(
        `
  INSERT INTO Notifications (user_id, type, message, status, context_id)
  SELECT user_id, 'match_found', $2, 'Unread', $1
  FROM Exchange_Requests
  WHERE request_id = ANY($3::int[])
    AND NOT EXISTS (
      SELECT 1 FROM Notifications
      WHERE user_id = Exchange_Requests.user_id
        AND type = 'match_found'
        AND context_id = $1
    )
`,
        [a, "נמצאה התאמה הדדית לבקשת ההחלפה שלך!", [a, b]]
      );

      // ✅ שליחת סוקט לכל משתמש שקשור לזוג הבקשות
      const { rows: involvedUsers } = await client.query(
        `
        SELECT user_id FROM Exchange_Requests
        WHERE request_id = ANY($1::int[])
      `,
        [[a, b]]
      );

      for (const row of involvedUsers) {
        emitCrossRequestMatch(row.user_id, {
          requestAId: a,
          requestBId: b,
          message: "🎯 נמצאה התאמה הדדית לבקשת ההחלפה שלך!",
        });
      }
    }
  } catch (err) {

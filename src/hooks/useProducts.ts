...
import { CreateProductPayload, DeleteProductPayload, IProduct, NUM_PRODUCTS_IN_PAGE, ProductCategory, UpdateProductPayload } from "@/types/products";
...
//   // const fetchUserProducts = async (userId: number): Promise<IProduct[]> => {
//   //   try {
//   //     const res = await axios.get(productRoutes.getProductsByUser(userId, NUM_PRODUCTS_IN_PAGE), {
//   //       headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
//   //     });
//   //     return res.data;
//   //   } catch (err: any) {
...
// ): Promise<IProduct[]> => {
//   try {
//     const res = await axios.get(productRoutes.getProductsByUser(userId, limit, offset), {
//       headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
//     });
//     return res.data;
//   } catch (err: any) {
//     console.error(err);
//     throw new Error(err?.response?.data?.error || "שגיאה בטעינת המוצרים של המשתמש");
//   } finally{
//     setLoading(false);
//   }
// };
const fetchUserProducts = async (
    ...
    try {
      ...
      const response = await axios.get( productRoutes.getProductsByUser(userId, limit, offset) + `&${params.toString()}`,
      {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
